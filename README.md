...
2. **Date started** (Housecall Pro job `created_at`, America/Chicago; dash when missing — never invented)
3. **Customer name**
4. **Job number**
5. **Primary tech**
...
Tablet and phone keep the same table (horizontal scroll if needed) so rows stay readable. No column is sticky — horizontal scroll moves every column, including #, Date started, Customer name, and Job number.
...
**Date started** is Housecall Pro’s job `created_at` (ISO 8601 on the Jobs API). Sync maps that field through `/api/jobs/hcp` and stores it as `hcpCreatedAt`. The board shows it as a shop-floor date in **America/Chicago** (for example `Sep 12, 2026`). There is no mapped HCP “started” field; `work_timestamps.started_at` is not used.
